 ...
 *
 */
public class RangeCorrectionRight extends Command {
    ...
    protected void initialize() {
    	minSweepAngle = -90 / 180;
		maxSweepAngle = 90 / 180;
		binIgnore = false;
    }
    ...
    protected void execute() {
    	...
    	while(sweepAngle <= maxSweepAngle){
    		RobotMap.servoRight.setAngle(sweepAngle);
    		inchesFront = RobotMap.rangeRight.getVoltage()/.00765;
    		if(inchesFront < 40){
    	...
    	while(RobotMap.rangeRight.getVoltage() / .00765 < 5){
    		RobotMap.driveTrainRobotDrive41.mecanumDrive_Cartesian(0, 1, 0, 0);
    	}
    	RobotMap.rightHook.set(DoubleSolenoid.Value.kForward);
    		}
    	}
    	}
    	}
