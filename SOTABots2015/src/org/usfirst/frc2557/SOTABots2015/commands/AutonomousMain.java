package org.usfirst.frc2557.SOTABots2015.commands;

import edu.wpi.first.wpilibj.command.CommandGroup;

/**
 *
 */
public class AutonomousMain extends CommandGroup {
    
    public  AutonomousMain() {
//    	addSequential(new BackHook());
//    	addSequential(new AutoDriveCommand(-1, 0, 0, 0, 15));
//    	addParallel(new IntakeClose());
//    	addSequential(new IntakeIn());
//    	addSequential(new LimitSwitchCommand());
//    	addSequential(new Grab());
//    	addParallel(new IntakeArm());
//    	addSequential(new LiftUp());
//    	addSequential(new RotateAutoCommand(-90));
//    	addSequential(new AutoDriveCommand(-1, 0, 0, 0, 10));
//    	addSequential(new RightHook());
//    	addSequential(new RotateAutoCommand(-180));
//    	addSequential(new AutoDriveCommand(1, 0, 0, 0, 15));
//    	addSequential(new Release());
//    	addSequential(new Set());
//    	addParallel(new IntakeClose());
//    	addSequential(new IntakeIn());
//    	addSequential(new LimitSwitchCommand());
//    	addSequential(new Grab());
//    	addParallel(new LiftUp());
//    	addSequential(new RotateAutoCommand(0));
//    	addSequential(new AutoDriveCommand(-1, 0, 0, 0, 20));
//    	addSequential(new Release());
//    	addSequential(new Set());
//    	addParallel(new IntakeClose());
//    	addSequential(new IntakeIn());
//    	addSequential(new LimitSwitchCommand());
//    	addSequential(new Grab());
//    	addParallel(new LiftUp());
//    	addSequential(new RotateAutoCommand(90));
//    	addSequential(new AutoDriveCommand(0, 1, 0, 0, 15));
//    	addSequential(new Set());
//    	addSequential(new AutoDriveCommand(0, -1, 0, 0, 5));
        // Add Commands here:
        // e.g. addSequential(new Command1());
        //      addSequential(new Command2());
        // these will run in order.

        // To run multiple commands at the same time,
        // use addParallel()
        // e.g. addParallel(new Command1());
        //      addSequential(new Command2());
        // Command1 and Command2 will run in parallel.

        // A command group will require all of the subsystems that each member
        // would require.
        // e.g. if Command1 requires chassis, and Command2 requires arm,
        // a CommandGroup containing them would require both the chassis and the
        // arm.
    }
}
